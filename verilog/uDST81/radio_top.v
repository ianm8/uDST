...
//
//   *** 81 MHz VARIANT (native 27 MHz crystal, x3 = 81 MHz). ***
//   For boards keeping the 27 MHz crystal instead of fitting a 20 MHz TCXO.
//   Sample chain: 81 MHz -> CIC /432 -> 187.5 kHz -> FIR /6 -> 31.25 kHz
//   (total /2592, same 31.25 kHz IQ rate as the 80 MHz build). The internal
//   clock nets are still named clk80/clk80_int to keep the .cst pin-constraint
//   names unchanged -- they carry 81 MHz here. The Gowin rPLL IP MUST be
//   REGENERATED for 27 MHz in -> 81 MHz out (VCO 648 MHz, ODIV 8; IDIV /1,
//   FBDIV x3). The Pico must compute NCO tuning words with f_clk = 81e6.
//
...
//
// Project sources: this file, DDC22.v, DUC18.v (its mac16_reg is commented out;
// DDC22.v provides the shared copy), pll_27_81.v, sin_lut.hex,
// fir_rx252_coeffs.hex. ODDR is a Gowin primitive.
// ============================================================================
module radio_top (
    input  wire        sys_clk,        // 27 MHz crystal (clock pin)

    input  wire        tx_rx,          // 0 = RX, 1 = TX

    input  wire signed [11:0] adc_in,  // AD9236, two's complement
    output wire        [9:0]  dac_out,
    output wire        clk80,          // 81 MHz forwarded to converters (net name kept for .cst)
    ...
    // ---- clocking: 27 MHz -> rPLL -> 81 MHz ----
    // REGENERATE the Gowin rPLL IP for 27 MHz in / 81 MHz out (x3): IDIV /1,
    // FBDIV x3, ODIV 8 -> VCO 648 MHz (within the 400-1200 MHz GW1N range).
    // clk80_int carries 81 MHz in this variant.
    wire clk80_int, pll_lock;
    pll_27_81 u_pll (.clkin(sys_clk), .clkout(clk80_int), .lock(pll_lock));
    ...
    //   high on iq_valid, low after 1296 cycles, until the next iq_valid.
    //   1296 = (81e6/31250)/2 = 2592/2  (was 1280 = 2560/2 at 80 MHz).
    //   Pico waits high, runs DSP, waits low, repeats.
    localparam [11:0] HB_HALF = 12'd1296;
    ...
    //   RX: step every 2^25 clk80 cycles  = 2.414 Hz  (~414 ms/step @81 MHz)
    //   TX: step every 2^23 clk80 cycles  = 9.656 Hz  (~104 ms/step @81 MHz)
