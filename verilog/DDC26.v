// ============================================================================
// DDC26.v  -- uDST receive DDC, 12-bit ADC, 80 MHz sample clock
//             Target: Gowin GW1NR-9 (Brisbane Silicon board).
//
// Chain:  NCO quadrature mixer -> 3-stage CIC decimate-by-512
//         -> 255-tap FIR decimate-by-5 -> 16-bit I/Q at 31250 Hz.
//         Total decimation 2560; 80 MHz / 2560 = 31250 Hz.
//
// Gain accounting (all confirmed in simulation):
//   quadrature mixing of a real signal   x0.5
//   CIC  512^3 = 2^27, output >>>22      x32
//   FIR  unity (Q15 coefficients sum to 32768, >>>15)
//   ------------------------------------------------
//   ADC codes -> I/Q codes               x16
//   So a full-scale 12-bit input (+/-2047) gives |I/Q| = 32734, i.e. the
//   digital clip point sits ~1 LSB below ADC full scale. One more bit of CIC
//   gain (>>>21) would clip 6 dB BEFORE the ADC does.
//
// THIS FILE IS COMMENT-ONLY UPDATED: the RTL is byte-for-byte the working
// 80 MHz build. Only comments were corrected (stale iCE40/SB_MAC16 references
// from the original prototype, and a few labels that no longer matched the
// code).
// ============================================================================
...
module nco_sincos_rx #(
    ...
    // The reads are REGISTERED: a Gowin BSRAM read is synchronous, so a
    // registered read maps cleanly to block RAM. An asynchronous
    // (combinational) read of a 1024x16 ROM does NOT infer BSRAM and is built
    // from a large, slow mux of logic cells instead.
...
// combinational multiply in one clock. Gowin infers the hard multiplier from
// this pattern automatically; if it ever falls back to LUT logic the Fmax
// drop is obvious in the timing report.
//
...
module mac16_reg (
    ...
    reg signed [15:0] a_r, b_r;         // input registers   -> DSP A/B input regs
...
// 255-tap, decimate-by-5, RX channel/anti-alias FIR.
// (Decimation, not interpolation -- 156250 Hz in, 31250 Hz out.)
//
...
//   - round register        -> +ROUND_LATENCY (the accI_rnd stage below)
//   We accumulate the product every clock and latch the result once the last
//   real tap (NTAPS-1) has come out AND the round register has caught up:
//   CNT_LAST = NTAPS + READ_LAT + MUL_LATENCY + ROUND_LATENCY = 260.
//
...
//
// IMPORTANT: fir_rx255_coeffs.hex must contain 256 words -- the 255 real taps
// PLUS a final 0000. Hmem[ZERO_TAP] is read on EVERY idle clock, so if that
// entry is missing the accumulator is poisoned permanently (X in simulation;
// it only survives on hardware because Gowin BSRAM initialises to zero).
module fir255_dec5_rx (
    ...
    initial for (bi = 0; bi < BUF_LEN; bi = bi + 1) begin
        dI[bi] = 0;
        dQ[bi] = 0;   // BSRAM powers up to 0 on Gowin; matched here for sim
    end
    ...
    localparam integer ACCW = 40;            // was 48; ~32 bits is the worst-case need
                                             // (32768 full-scale in x ~2^15 coeff sum)
    ...
    // Round-half-up in its OWN register stage (this is the ROUND_LATENCY=1
    // above). The register matters: putting the 40-bit add on the same clock as
    // the accumulate cost ~20 MHz of Fmax when it was first tried. Saturation
    // is separate, below.
    ...
    // Saturate-then-slice: honest clipping rather than a silent wrap. With the
    // CIC at >>>22 the FIR input can already be at full scale, so this can fire.
...
module ddc_top (
    ...
    // ============================================================
    // INLINE REGISTERED MULTIPLY (inferred Gowin DSP block)
    // ============================================================
    // Written out inline rather than instantiated, so the same source builds
    // on any device. The input/output register pattern is what lets Gowin pack
    // it into the hard multiplier.
    ...
    // ============================================================
    // SCALE MIXER OUTPUTS (Q15 -> integer, round-half-up)
    // ============================================================
    ...
    // ============================================================
    // CIC INTEGRATORS  (3 stages, decimate-by-512)
    // ============================================================
    // Integrators and combs MUST be the same width. A CIC is only correct
    // because the integrators' modulo-2^W wraparound is exactly undone by the
    // combs; if the combs were wider, every wrap would be sign-extended into
    // the wider arithmetic instead of cancelling. 48 bits both sides here.
    // Range check: |mix| <= 2048, R^N = 512^3 = 2^27 -> 2.75e11 -> 39 bits
    // incl. sign, so 48 has plenty of margin.
    ...
    // ============================================================
    // Decimation control: free-running 9-bit counter, /512
    // ============================================================
    reg [8:0] dec_cnt;
    ...
    // registered cic_valid and registered outputs
    reg        cic_valid_reg;
    ...
    always @(posedge clk80) begin
        if (rst) begin
            ...
            if (dec_cnt == 9'd511) begin
                ...
                // Stage E: round, scale, register the output, then assert valid.
                // Shift sets the CIC gain: 512^3 = 2^27, so >>>S gives 2^(27-S).
                //   >>>23 = x16   >>>22 = x32   >>>21 = x64   >>>20 = x128
                // The round constant must always be 2^(S-1) -- if the shift is
                // changed, change the constant on the same line to match.
                // Currently >>>22: full-scale ADC gives |I/Q| = 32734, so the
                // digital clip point is level with the ADC's. >>>21 would clip
                // 6 dB before the ADC.
    ...
    // NOTE the one-sample offset: cicI/cicQ reload one clock AFTER cic_valid
    // goes high, so the FIR stores the PREVIOUS decimated sample. That is a
    // constant 1/156250 s delay on the whole stream and is harmless -- it is
    // documented here only so it isn't rediscovered as a bug later.
    wire cic_valid = cic_valid_reg;
    // saturate 32-bit CIC result to 16 bits for the FIR (clip, don't wrap)
